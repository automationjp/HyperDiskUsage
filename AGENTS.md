...
`cargo check --workspace` gives a fast compile sanity pass. Run `cargo fmt --check` to enforce formatting and `cargo clippy --workspace --all-targets` to lint. Clippy takes the MSRV from each crate's own `rust-version`, since `hyperdu-mcp` needs 1.88 for rmcp while the other three still build on 1.75. Every other package field (version, edition, license, repository, homepage) and the `hyperdu-core` dependency pin are inherited from `[workspace.package]` / `[workspace.dependencies]` in the root `Cargo.toml`, so a release bumps `version` there (twice: the package field and the dependency pin) and nowhere else. Scripts that need the version ask `cargo pkgid -p hyperdu-cli` instead of grepping a manifest. Publish order is `hyperdu-core` first, then `hyperdu-cli`, `hyperdu-mcp`, `hyperdu-gui`.
