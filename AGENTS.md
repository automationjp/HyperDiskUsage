...
`cargo check --workspace` gives a fast compile sanity pass. Run `cargo fmt --check` to enforce formatting and `cargo clippy --workspace --all-targets` to lint. Clippy takes the MSRV from each crate's own `rust-version`, since `hyperdu-mcp` needs 1.88 for rmcp while the other three still build on 1.75. Every other package field (version, edition, license, repository, homepage) and the `hyperdu-core` dependency pin are inherited from `[workspace.package]` / `[workspace.dependencies]` in the root `Cargo.toml`, so a release bumps `version` there (twice: the package field and the dependency pin) and in no crate manifest. Scripts that need the version ask `cargo pkgid -p hyperdu-cli` instead of grepping a manifest; pkgid answers from `Cargo.lock`, so `cargo update -w` belongs to the same commit as the bump. Publish order is `hyperdu-core` first, then `hyperdu-cli`, `hyperdu-mcp`, `hyperdu-gui`.

The packaging and documentation tree carries versions of its own, and an earlier version of this file claimed it did not — which is how `snap/snapcraft.yaml`, its generator, and `plugin/plugin.json` sat at 0.4.0 while the workspace moved to 0.5.0-beta.2. `scripts/lint/versions.sh` is now the list, and it splits those files in two: the ones describing this tree (man page, snap manifest, plugin manifest) track `[workspace.package]` and are rewritten by `--fix`, while the ones naming published artefacts (`README.md`, the crate READMEs, `site/_config.yml`, `bucket/hyperdu.json`) track the last release and must not be bumped before it exists, or they advertise downloads that 404. It runs unconditionally from `scripts/lint/run.sh`, so CI, pre-commit and `scripts/release/tag.sh` all enforce it. A release is therefore: bump `Cargo.toml`, `cargo update -w`, `bash scripts/lint/versions.sh --fix`, tag with `scripts/release/tag.sh` — which refuses a tag that does not name the workspace version — and once the assets exist, point the release group at them, `bucket/hyperdu.json` including the new SHA256.
