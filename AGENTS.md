...
`cargo check --workspace` gives a fast compile sanity pass. Run `cargo fmt --check` to enforce formatting and `cargo clippy --workspace --all-targets` to lint. Clippy takes the MSRV from each crate's own `rust-version`, since `hyperdu-mcp` needs 1.88 for rmcp while the other three still build on 1.75. Every other package field (version, edition, license, repository, homepage) and the `hyperdu-core` dependency pin are inherited from `[workspace.package]` / `[workspace.dependencies]` in the root `Cargo.toml`, so a release bumps `version` there (twice: the package field and the dependency pin) and in no crate manifest. Scripts that need the version ask `cargo pkgid -p hyperdu-cli` instead of grepping a manifest. Run `cargo update -w` in the same commit as a version bump so `Cargo.lock` stays synchronized. Publish order is `hyperdu-core` first, then `hyperdu-cli`, `hyperdu-mcp`, `hyperdu-gui`.

The packaging and documentation tree also carries version literals. `scripts/lint/versions.sh` is the authoritative inventory: files describing the current tree (man page, snap manifest/generator, plugin manifest, winget generator) track the workspace version, while files naming downloadable assets track the latest published release and may legitimately lag between a version bump and release publication. Run `bash scripts/lint/versions.sh --fix` for the workspace group only; never bump the published-release group until the corresponding assets exist. `scripts/lint/run.sh` executes this gate before Cargo linting so a stale `Cargo.lock` cannot be silently refreshed first. `scripts/release/tag.sh` also rejects tags that do not equal the locked workspace version.
