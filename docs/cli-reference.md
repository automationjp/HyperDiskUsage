...
`--mft` はNTFSボリュームのルート（例: `C:\`）と管理者権限を必要とする実験的機能です。変更中のボリュームで通常列挙との完全一致は保証しません。利用条件を満たさない場合や、読取・解析を完了できない場合は通常の列挙へ戻ります。
...
ディレクトリの一覧取得権限を確認し、アクセス拒否された配下は集計しません。symlink と junction も除外します。未対応の reparse metadata や、アクセス拒否と複数ハードリンクが重なり可視性を確定できない場合は通常列挙へ戻します。
