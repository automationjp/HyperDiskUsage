...
| `-d N` / `--max-depth N` | 省略時は無制限 | 出力する深さの上限。ルート（指定したパス）が `0` です。GNU `du` の `-d` と同一で、表示のみを制限し、走査は最下層まで行うため各行の合計値は完全です。`0` はルート行のみ（`du -d 0` = `--summarize` と同じ）、`1` はルート直下まで。**無制限を表す値はありません。オプション自体を省略します。** 値は `du` と同じ base-0 解釈（`010` は8進で 8、`0x2` は 16、先頭 `+` と空白は許容、上限は `u64::MAX`）。不正値は `du` と同じ文面で標準エラーに出力し、終了コード `1` で失敗します。重複指定は最後が勝ちます。入れ子のパスを複数指定した場合、深さは**左端の**該当パスから数えます。 |
| `--prune-depth N` | 省略時は無制限 | 走査を打ち切る深さ。`--max-depth` と異なり走査そのものを止めるため、**打ち切った下は合計値からも消え、実際の使用量より小さく表示されます**。指定時は標準エラーに警告を出します。`--max-depth` とは併用できません。`du` に対応するオプションはない HyperDU 独自の指定です。 |
...
| 引数 | 既定値 | 挙動 |
|---|---|---|
| `--compat MODE` | `hyperdu` | `gnu`, `gnu-strict`, `posix-strict` で `ブロック数<TAB>パス` の出力を選択します。行は `du` と同じ深さ優先の後順で、**子が親より先に出力され、ルート自身の行が最後**になります（`du DIR \| tail -1` が合計になる並びです）。兄弟の順序は `du` では列挙順（ファイルシステム依存）ですが、HyperDU では再現性のためソート順に固定しています。パスの区切り文字は指定したルートの綴りに従います（`C:/x` を指定すれば子も `/`、`C:\x` なら `\`）。正常終了時は `du` と同様に標準エラーへ情報を出しません（合計値の正しさに関わる警告は出ます）。 |
