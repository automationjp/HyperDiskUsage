...
struct Args {
    ...
    #[arg(
        long = "mft",
        action = ArgAction::SetTrue,
        long_help = "【既知の不具合】現在このモードはファイルを大幅に取りこぼします。\n\
    実測で列挙が 10,220,623 件のところ 1,134,722 件しか数えませんでした（-88.9%）。\n\
    断片化した $MFT の実行リストを最後まで辿れていないためです。#15 を参照。\n\
    正確な値が必要な場合はこのオプションを使わないでください。\n\
    \n\
