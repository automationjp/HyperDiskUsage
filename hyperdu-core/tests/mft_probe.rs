...
fn dump(rec: &[u8]) {
    ...
    eprintln!("probe: header={header:?} sequence={} base={:#x}", u16::from_le_bytes(rec[16..18].try_into().unwrap()), u64_at(rec, 32));
    for a in mft::Attributes::new(rec, &header) {
        ...
        eprintln!("probe: attribute={a:?} name_units={} instance={}", rec[a.pos + 9], u16::from_le_bytes(rec[a.pos+14..a.pos+16].try_into().unwrap()));
        if a.non_resident {
...
fn actual_mft_is_complete() {
    ...
    for number in 16..reader.record_count() {
        ...
        for a in mft::Attributes::new(&rec, &header) {
            if a.type_code != mft::attr_type::ATTRIBUTE_LIST || a.non_resident { continue; }
            ...
            while p + 26 <= value.len() {
                ...
                if len < 26 || p + len > value.len() { break; }
                ...
                if kind == mft::attr_type::DATA && reference & ((1 << 48) - 1) != number {
                    if let Some(ext) = reader.read_record(reference & ((1 << 48) - 1)) { dump(&ext); }
                    else { eprintln!("probe: extension unreadable"); }
                }
                p += len;
            }
        }
